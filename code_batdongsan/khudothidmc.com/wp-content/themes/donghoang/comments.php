...
<?php if ( post_password_required() ) : 
 ...
 if (comments_open() ) :
	?>
		<p><?php _e( 'Bài đăng này là mật khẩu bảo vệ. Nhập mật khẩu để xem bất kỳ ý kiến.', 'avia_framework' ); ?></p>
	<?php
endif;
...
<?php 		
		if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // Are there comments to navigate through?
			...
			echo "<span class='comment_next_page'>";
				 next_comments_link( __( 'Bình luận mới hơn &raquo;', 'avia_framework' ) );
			echo "</span>";
			echo "</span>";
...
<?php 
		if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // Are there comments to navigate through?
			...
			echo "<span class='comment_prev_page'>";
				 previous_comments_link( __( '&laquo; Bình luận cũ hơn', 'avia_framework' ) );
			echo "</span>";
			echo "<span class='comment_next_page'>";
				 next_comments_link( __( 'Bình luận mới  &raquo;', 'avia_framework' ) );
			echo "</span>";
			echo "</span>";
		endif; // check for comment navigation
	 ...
	 */
	 if(comments_open()){
		 ...
		 echo "<h3 class='miniheading'>".__('Để lại câu trả lời','avia_framework')."</h3>";
		 echo "<span class='minitext'>".__('Bạn muốn tham gia các cuộc thảo luận?','avia_framework')." <br/>".__('Tham gia miễn phí để đóng góp !','avia_framework')."</span>";
		 comment_form();
		 echo "</div>";
	 }
	 else if(get_comments_number())
	 {
		 ...
		 */
	 	...
	 	echo "<h3 class=' commentsclosed'>".__( 'Những ý kiến đóng góp !.', 'avia_framework' )."</h3>";
	 } 
