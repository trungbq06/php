...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
		...
		function form($instance) {
			...
			$instance = wp_parse_args( (array) $instance, array('url' => 'https://www.facebook.com/minhhunglandhn', 'title' => '') );
...
if (!class_exists('avia_combo_widget'))
{
	class avia_combo_widget extends WP_Widget {
		...
		function widget($args, $instance)
		{
			...
			echo '<div class="tab first_tab active_tab widget_tab_popular"><span>'.__('Xem nhiều nhất', 'avia_framework').'</span></div>';
			echo "<div class='tab_content active_tab_content'>";
			avia_get_post_list('cat=&orderby=comment_count&posts_per_page='.$posts);
			echo "</div>";

			echo '<div class="tab widget_tab_recent"><span>'.__('Mới Nhất', 'avia_framework').'</span></div>';
			echo "<div class='tab_content'>";
			avia_get_post_list('showposts='. $posts .'&orderby=post_date&order=desc');
			echo "</div>";

			echo '<div class="tab widget_tab_comments"><span>'.__('Bình luận', 'avia_framework').'</span></div>';
			echo "<div class='tab_content'>";
			avia_get_comment_list( array('number' => $posts, 'status' => 'approve', 'order' => 'DESC') );
			echo "</div>";

			echo '<div class="tab last_tab widget_tab_tags"><span>'.__('Thẻ', 'avia_framework').'</span></div>';
