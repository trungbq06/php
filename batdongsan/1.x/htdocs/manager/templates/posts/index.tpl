			...
			<table class="table table-striped table-bordered bootstrap-datatable datatable">
			  ...
			  <tbody>
				...
				<tr>
					...
					<td><a href="posts?id={$value.category_detail}">{$value.category}</a></td>
					<td class="center">
                         <a href="#" table="tbl_posts" value="{$value.id}" class="label frm_active {if $value.active==1}label-success{else}label-warning{/if}" title="Click to Active/Inactive  this filed">{$value.active_view}</a>
					</td>
					<td class="center">
						<a title="View" href="?mod=posts&site=view&id={$value.id}"><i class="icon-zoom-in"></i></a> &nbsp;&nbsp;|&nbsp;&nbsp; 
						<a title="Delete" href="#" link="?mod=helps&site=delete&table=posts&id={$value.id}" class="confirmer"><i class="icon-trash"></i></a>
					</td>
				</tr>
                {/foreach}
			  </tbody>
